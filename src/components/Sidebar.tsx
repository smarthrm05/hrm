...
const menuItems = [
  ...
  { id: 'kelola-karyawan', label: 'Kelola Karyawan', icon: Users, isEmployeeGroup: true },
  { id: 'kehadiran', label: 'Kehadiran', icon: Clock, isAttendanceGroup: true },
  { id: 'request-absen', label: 'Request Absen', icon: MapPinCheck },
  { id: 'cuti', label: 'Cuti', icon: CalendarClock },
  { id: 'izin', label: 'Izin', icon: FileText },
  { id: 'lembur', label: 'Lembur', icon: CalendarClock },
  { id: 'penggajian', label: 'Penggajian', icon: DollarSign },
  { id: 'jadwal-shift', label: 'Jadwal Shift', icon: Calendar, isShiftGroup: true },
              ...
              return (
                <div key={item.id}>
                  ...
                  >
                    ...
                    {isAttendanceOpen ? <ChevronDown className="h-4 w-4" /> : <ChevronRight className="h-4 w-4" />}
                  </Button>

                  <div className={cn('transition-all duration-300 ease-in-out overflow-hidden',
                    isAttendanceOpen ? 'max-h-96 opacity-100 mt-2' : 'max-h-0 opacity-0')}>
                      ...
                      {[
                        ...
                        <button
                          ...
                          className={cn(
                            ...
                            currentPath === sub.id ? 'bg-white text-blue-900 font-semibold' : 'text-gray-800 hover:bg-gray-100'
                          )}
                        >
                          {sub.label}
                        </button>
              ...
              return (
                <div key={item.id}>
                  ...
                  >
                    ...
                    {isShiftOpen ? <ChevronDown className="h-4 w-4" /> : <ChevronRight className="h-4 w-4" />}
                  </Button>

                  <div className={cn('transition-all duration-300 ease-in-out overflow-hidden',
                    isShiftOpen ? 'max-h-96 opacity-100 mt-2' : 'max-h-0 opacity-0')}>
                    <div className="ml-6 bg-white rounded-xl py-2 px-3 space-y-1 shadow-md">
                      {[
              ...
              return (
                <div key={item.id}>
                  ...
                  >
                    ...
                    {isEmployeeOpen ? <ChevronDown className="h-4 w-4" /> : <ChevronRight className="h-4 w-4" />}
                  </Button>

                  <div className={cn('transition-all duration-300 ease-in-out overflow-hidden',
                    isEmployeeOpen ? 'max-h-96 opacity-100 mt-2' : 'max-h-0 opacity-0')}>
                    <div className="ml-6 bg-white rounded-xl py-2 px-3 space-y-1 shadow-md">
                      {[
