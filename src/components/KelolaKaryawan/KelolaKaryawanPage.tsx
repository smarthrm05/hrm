...
export const DataKaryawanPage = () => {
  ...
  const isKontrakAkanHabis = (selesaiKontrak: string): boolean => {
    const today = new Date();
    const endDate = new Date(selesaiKontrak);
    const diffTime = endDate.getTime() - today.getTime();
    const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24));
    return diffDays > 0 && diffDays <= 90; // Kontrak akan habis dalam 90 hari
  };
  ...
  const totalAktif = data.filter(k => k.statusKerja === 'Aktif').length;
  const totalTidakAktif = data.filter(k => k.statusKerja === 'Tidak Aktif' || isKontrakHabis(k.selesaiKontrak)).length;
  const totalAkanHabisKontrak = data.filter(k => isKontrakAkanHabis(k.selesaiKontrak) && k.statusKerja === 'Aktif').length;
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      {/* Summary Cards dengan border kiri */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        {/* Card Karyawan Akan Habis Kontrak */}
        <Card className="bg-white border-l-4 border-yellow-500 shadow-md">
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div className="space-y-1">
                <p className="text-sm font-medium text-gray-700">Total Karyawan Akan Habis Kontrak</p>
                <p className="text-3xl font-bold text-gray-900">{totalAkanHabisKontrak}</p>
