  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (username.trim() && password.trim()) {
      Swal.fire({
        ...
        confirmButtonText:
          '<span style="color: #196de3; font-weight: bold;">OK</span>',
        customClass: {
          popup: 'rounded-xl',
          title: 'text-xl',
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-white p-4 sm:p-6">
      <div className="w-full max-w-full sm:max-w-5xl mx-auto">
        ...
        >
          {/* Card login geser ke kiri di desktop */}
            ...
            <Card className="w-full max-w-full sm:max-w-md bg-white rounded-2xl shadow-2xl p-4 sm:p-8">
              <CardHeader className="space-y-1 text-center pb-4 sm:pb-6">
                <div className="flex items-center justify-center space-x-2">
                  <Users className="text-blue-600 h-7 sm:h-8 md:h-10 w-7 sm:w-8 md:w-10" />
              ...
              <CardContent>
                ...
                >
                    ...
                    <div className="inline-flex bg-gray-100 p-1 sm:p-2 rounded-full border border-gray-300 shadow-inner text-xs sm:text-sm md:text-base">
                      <button
                        ...
                        className={`px-2 sm:px-3 py-1 sm:py-1.5 font-medium rounded-full flex items-center gap-1 transition ${
                          role === 'management'
                            ? 'bg-[#196de3] text-white shadow'
                            : 'text-gray-500 hover:text-blue-600'
                        }`}
                      >
                        <Briefcase
                          className={`w-4 sm:w-5 md:w-6 h-4 sm:h-5 md:h-6 ${
                            role === 'management' ? 'text-white' : 'text-gray-500'
                          }`}
                        />
                        Manajemen
                      </button>
                      ...
                      >
                        <UserCircle
                          className={`w-4 sm:w-5 md:w-6 h-4 sm:h-5 md:h-6 ${
                            role === 'employee' ? 'text-white' : 'text-gray-500'
                          }`}
                        />
                        Karyawan
                      </button>
                  ...
                  {/* Submit Button (tetap fix, tidak ikut transisi) */}
                  <Button
                    type="submit"
                  ...
                  >
                    <Users className="w-4 sm:w-5 md:w-6 h-4 sm:h-5 md:h-6 mr-2" />
                    Masuk ke Sistem
                  </Button>
                </form>
              </CardContent>
